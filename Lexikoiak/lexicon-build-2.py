...
def lexicon_build(textname=None, doclink=None):
    ...
    for span in spans:
        ...
        for paragraph in span_content.split('{{aingura|'):
            ...
            print(f"Processing new paragraph in language {lang}, with {len(tokens)} tokens:")
            print(tokens)
            token_num = 0
            while token_num < len(tokens):
                token = tokens[token_num]
                if re.search(r"[^\w\-]", token) or re.search(r"\d", token):
                    print(f"\nSkipping token: {token} with number {token_num}")
                    token_num += 1
                    continue
                doc_token_count += 1
                print(f"\nNow processing token: {token} with number {token_num} ({doc_token_count}. processed token in document)")
                ...
                if token_num == 0:
                    ...
                    context_left = " ".join(tokens[:token_num])
                else:
                    context_left = " ".join(tokens[token_num-8:token_num])

                if len(tokens) - token_num < 8:
                    context_right = " ".join(tokens[token_num+1:len(tokens) - 1])
                else:
                    context_right = " ".join(tokens[token_num+1:token_num + 9])

                context = f"{context_left}\t{token}\t{context_right}"

                print_context = re.sub(r'@', '', context)
                print_context = re.sub(r' {2,}', ' ', print_context)
                print_context = f"... {print_context} ..."
                print(f"context in {aingura_link}\n'{print_context}'")


                #save to document lexicon
                token_dict = {'doc_token_count': doc_token_count, 'lemma': lemma_candidate, 'aingura': aingura_link, 'context': print_context}
                if token.lower() not in lexicon[lang]:
                    lexicon[lang][token.lower()] = [token_dict]
                else:
                    lexicon[lang][token.lower()].append(token_dict)
                token_num += 1
    return lexicon
